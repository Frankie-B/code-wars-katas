...
const e = [{ a: [1, 2] }, { a: [1, 3] }, { a: [1, 4] }];

e.forEach(el => {
    const sum = el.a[0] + el.a[1];
...
[
    ...
    const percentage = Math.floor((el.cars.broken / el.cars.total) * 100);
    const goodCars = el.cars.total - el.cars.broken;
    const brokeCars = el.cars.broken;
...
autos.forEach(el => {
    const { unknown } = el.cars.broken.reason;
    const engineProb = el.cars.broken.reason.engine;
    const totalBrokenCars = [unknown + engineProb];
    const spots = el.cars.broken.reason.parkingspot;
