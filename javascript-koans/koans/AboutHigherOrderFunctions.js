...
describe('About Higher Order Functions', function() {
    it('should use filter to return array items that meet a criteria', function() {
        const numbers = [1, 2, 3];
        const odd = _(numbers).filter(function(x) {
            return x % 2 !== 0;
        });
    ...
    it("should use 'map' to transform each element", function() {
        const numbers = [1, 2, 3];
        const numbersPlus1 = _(numbers).map(function(x) {
            return x + 1;
        });
    ...
    it("should use 'reduce' to update the same result on each iteration", function() {
        const numbers = [1, 2, 3];
        const reduction = _(numbers).reduce(function(/* result from last call */ memo, /* current */ x) {
            return memo + x;
        }, /* initial */ 0);
